...
container_create( const char *logical, mode_t mode, int flags, pid_t pid )
{
    ...
    stringstream ss;
    ss << pconf->file_backend << "/" << pconf->index;
    pconf->index++;
    string real_path = ss.str();
    ret =  Container::create(real_path,Util::hostname(),mode,flags,
                             &attempt,pid,expansion_info.mnt_pt->checksum,
                             lazy_subdir);
    Util::Symlink(real_path.c_str(), path.c_str());
    PLFS_EXIT(ret);
}
...
//    PLFS_EXIT(-ENOSYS);
    ExpansionInfo exp_info;
    string toPath = expandPath(to, &exp_info, EXPAND_CANONICAL, -1, 0);
//    string path = expandPath(logical, &exp_info, EXPAND_CANONICAL, -1, 0);
    ret = retValue(Util::Symlink(realpath(path.c_str(), NULL),toPath.c_str()));
    mlog(PLFS_DAPI, "%s: %s to %s: %d", __FUNCTION__,
            path.c_str(), toPath.c_str(),ret);
    PLFS_EXIT(ret);
