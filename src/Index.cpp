...
void * IndexFileInfo::listToStream(vector<IndexFileInfo> &list,int *bytes)
{
    ...
    for(itr=list.begin();itr!=list.end();itr++){
        ...
        mlog(IDX_DCOMMON, "Size of hostname is %d\n",len);
...
Index::~Index() {
    ...
    mlog(IDX_DCOMMON, "There are %ld chunks to close fds for\n", chunk_map.size());
...
void *Index::mapIndex( string hostindex, int *fd, off_t *length ) {
    ...
    *fd = Util::Open( hostindex.c_str(), O_RDONLY );
    ...
    if ( *length == 0 ) {
        mlog(IDX_DRARE, "%s is a zero length index file\n", hostindex.c_str() );
        return NULL;
    }
    if (length < 0) {
        mlog(IDX_DRARE, "%s WTF lseek: %s\n", __FUNCTION__, strerror(errno));
        return (void*)-1; 
    }
...
int Index::readIndex( string hostindex ) {
    ...
    for( size_t i = 0; i < entries; i++ ) {
        ...
        if( known_chunks.find(h_entry.id) == known_chunks.end() ) {
            ...
            mlog(IDX_DCOMMON, "Inserting chunk %s (%ld)\n", cf.path.c_str(),
                chunk_map.size());
        }
    ...
    mlog(IDX_DAPI, "After %s in %p, now are %ld chunks\n",
...
int Index::debug_from_stream(void *addr){
    ...
    Util::tokenize((char*)addr,"\n",chunk_paths); // might be inefficient...
    for( size_t i = 0; i < chunk_paths.size(); i++ ) {
        mlog(IDX_DCOMMON, "Chunk path:%ld is :%s\n",i,chunk_paths[i].c_str());
    }
    return 0;
}
...
int Index::global_to_stream(void **buffer,size_t *length) {
    ...
    for(unsigned i = 0; i < chunk_map.size(); i++ ) {
        ...
        mlog(IDX_DCOMMON, "%s: constructed %s from %s\n", __FUNCTION__, 
    ...
    *buffer = malloc(*length);
    ...
    if(!buffer){
        mlog(IDX_DCOMMON, "%s, Malloc of stream buffer failed\n",__FUNCTION__);
        return -1;
    }
        ...
        pair<map<off_t,ContainerEntry>::iterator, bool> &insert_ret ) 
{
    ...
    for(first=insert_ret.first;;first--) {
        if (!first->second.overlap(incoming)) {  // went too far
            mlog(IDX_DCOMMON, "Moving first %lld forward, "
                    "no longer overlaps with incoming %lld\n",
...
int Index::insertGlobal( ContainerEntry *g_entry ) {
    ...
    mlog(IDX_DCOMMON, "Inserting offset %ld into index of %s\n",
        ...
        const char *last_func, const char *indexfile )
{
    ...
    if ( maddr != NULL && maddr != (void*)-1 ) {
        ...
        if ( ret2 < 0 ) {
            mlog(IDX_DRARE, "WTF. readIndex failed during munmap of %s (%lu): %s",
    ...
    if ( maddr == (void*)-1 ) {
        mlog(IDX_DRARE, "mmap failed on %s: %s\n",indexfile,strerror(errno));
    }
        ...
        off_t shift, string &path, pid_t *chunk_id, ContainerEntry *entry ) 
{
    ...
    *chunk_id   = entry->id;
    if( cf_ptr->fd < 0 ) {
        ...
        mlog(IDX_DCOMMON, "Not opening chunk file %s yet\n", cf_ptr->path.c_str());
    }
        ...
        double begin_timestamp, double end_timestamp ) 
{
            ...
            == offset )
    {
        ...
        if(chunk_map.size()==0){
            ...
            //mlog(IDX_DCOMMON, "The hostIndex logical path is: %s\n",cf.path.c_str());
...
void Index::truncate( off_t offset ) {
    ...
    bool first = false;
    mlog(IDX_DAPI, "Before %s in %p, now are %ld chunks\n",
