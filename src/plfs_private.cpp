           ...
           expansionMethod hash_method, int which_backend, int depth)
{
    ...
    char * tmpPath;
    string expanded = exp_info->backend + "/dir_root" + remaining;
    exp_info->dir_root_path = expanded;
    if ((tmpPath = realpath(expanded.c_str(), NULL)) != NULL) {
        expanded = string(tmpPath);
        free(tmpPath);
    }
    exp_info->expanded = expanded;
    ...
    if ( ! pconf ) {    // not yet initialized.  Try to do so.
        ...
        if (pconf) { // someone beat us in race.  they will initialize.
            ...
            if ( !pconf ) {
                ...
                std::cout << "Reaching interesting part" << std::endl;

                std::cout << "After mount" << std::endl;
                set<string> backends = pconf->backends;
                set<string>::iterator it;
                std::cout << "After itr" << std::endl;
                for (it = backends.begin(); it != backends.end(); it++) {
                    int dircount = 0;
                    string dir_root = string(*it + "/dir_root/");
                    string file_root = string(*it + "/file_root/");
                    DIR * dir_p;
                    struct dirent * d;
                    stringstream ss;
                    Util::Mkdir(dir_root.c_str(), DEFAULT_MODE);
                    Util::Mkdir(file_root.c_str(), DEFAULT_MODE);
                    Util::Opendir(file_root.c_str(), &dir_p);
                    while (Util::Readdir(dir_p, &d) == 0) {
                       dircount++;
                    } 
                    pconf->backend_id = dircount;
                    ss << dircount;
                    string instance_dir = string(file_root + ss.str());
                    Util::Mkdir(instance_dir.c_str(), DEFAULT_MODE);
                    pconf->file_backend = instance_dir; 
                    pconf->index = 0;
                }
            }
        }
