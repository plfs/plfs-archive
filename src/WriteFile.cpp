...
struct OpenFd * WriteFile::getFd( pid_t pid ) {
	...
	*/
        ...
        if ( fds.size() > 0 ) {
            ...
            mlog(WF_DCOMMON, "%s no fd to give to %d\n", __FILE__, (int)pid);
            ofd = NULL;
        }
        */
        mlog(WF_DCOMMON, "%s no fd to give to %d\n", __FILE__, (int)pid);
...
int WriteFile::removeWriter( pid_t pid ) {
    ...
    if ( ofd == NULL ) {
        ...
        // gets cleaned up
        mlog(WF_CRIT, "%s can't find pid %d\n", __FUNCTION__, pid );
        assert( 0 );
    } else {
        ofd->writers--;
        if ( ofd->writers <= 0 ) {
        ...
        if ( ret >= 0 ) {
            ...
            if (write_count%1024==0 && write_count>0) {
                ...
                if(index->memoryFootprintMBs() > index_buffer_mbs) {
                    index->stopBuffering();
                    mlog(WF_DCOMMON, "The index grew too large, no longer buffering");
                }
            }
...
int WriteFile::openIndex( pid_t pid ) {
    ...
    if ( fd < 0 ) {
        ...
        Util::MutexUnlock(&index_mux, __FUNCTION__);
        mlog(WF_DAPI, "In open Index path is %s\n",index_path.c_str());
