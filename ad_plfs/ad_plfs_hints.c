    ...
    if ((fd->info) == MPI_INFO_NULL) {
        ...
        if (users_info != MPI_INFO_NULL) {
            ...
            if (flag) {
                broadcast = 0;
                if(rank==0){
                    printf("Rank zero setting broadcast is %d\n");
                }
                tmp_val = broadcast;
                MPI_Bcast(&tmp_val, 1, MPI_INT, 0, fd->comm);
                if (tmp_val != broadcast) {
                    FPRINTF(stderr, "ADIOI_PLFS_SetInfo: the value for key \"plfs_disable_broadcast\" must be the same on all processes\n");
